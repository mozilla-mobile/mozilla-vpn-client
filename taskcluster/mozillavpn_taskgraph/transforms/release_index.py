...
def add_pr_number(config, tasks):
   for task in tasks:
        if task.__contains__("push-to-release-index"):
            release_index = task.pop("push-to-release-index")
            if release_index and int(config.params["level"]) == 3:
                name = task['name'].split("/")[0]
                version = config.params["app_version"]
                route = f"index.mozillavpn.v2.mozillavpn.releases.{version}.{name}.latest"
