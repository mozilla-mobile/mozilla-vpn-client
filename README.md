...
may look like `cmake -S . -B build -GXcode -DCMAKE_PREFIX_PATH=/Users/[your account]/Qt/6.4.1/macos/lib/cmake`)
...
```
export QT_IOS_BIN=/Users/[username]/Qt/6.4.1/ios/bin
```

Then generate the Xcode project using our script (and an optional adjust token):

```bash
./scripts/macos/apple_compile.sh ios [--adjust <adjust_token>] -q ~/Qt/6.4.1/macos/bin
```
...
3. Set the `QT_HOST_PATH` environment variable to point to the location of the `androiddeployqt` tool  -- minus the `/bin` suffix i.e. if `$(which androiddeployqt)` is `$HOME/Qt/6.4.1/gcc_64/bin/androiddeployqt`, `QT_HOST_PATH` is `$HOME/Qt/6.4.1/gcc_64/`.
 ...
 * Sample .env file:
  ```
  export PATH=$PATH:~/Qt/6.4.1/macos/bin:$PATH
  export QT_MACOS_BIN=~/Qt/6.4.1/macos/bin
