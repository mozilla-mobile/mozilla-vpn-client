...
Whether you’re traveling, using public WiFi, or simply looking for more online security, we will always put your privacy first.
...
* Read the docs in the [docs folder](https://github.com/mozilla-mobile/mozilla-vpn-client/tree/main/docs) or [in the wiki](https://github.com/mozilla-mobile/mozilla-vpn-client/wiki)
* [Mozilla support
  pages](https://support.mozilla.org/en-US/products/firefox-private-network-vpn)
...
For information on building see [docs](./docs/index.md).
...
Functional tests require a dummy build of the application, which is not built by default.
To build the `dummyvpn` target, in the root folder of this repository run:

```
cmake --build build -j$(nproc) --target dummyvpn
```

This will create a dummy build under the `build/tests/dummyvpn` folder.

#### Other dependencies
* Ensure your conda environment is active as it has node installed (`conda activate vpn`)
...
```
npm run functionalTest path/to/testFile.js
```

To run, say, the authentication tests:

```
npm run functionalTest tests/functional/testAuthenticationInApp.js
```
