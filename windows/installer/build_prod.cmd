...
:msi
	...
	echo [+] Compiling %1
	"%WIX%bin\heat" dir ..\..\unsigned\addons -o addon.wxs -scon -sfrag -srd -sreg -gg -cg addons -dr MozillaVPNAddonFolder || exit /b %errorlevel%
	"%WIX%bin\candle" %WIX_CANDLE_FLAGS% -dPlatform=%1 -arch %1 MozillaVPN.wxs addon.wxs || exit /b %errorlevel%
	echo [+] Linking %1
	"%WIX%bin\light" %WIX_LIGHT_FLAGS% -out "%~1/MozillaVPN.msi" -b ..\..\unsigned\addons MozillaVPN.wixobj addon.wixobj || exit /b %errorlevel%
	goto :eof
