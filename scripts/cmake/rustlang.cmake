...
function(__rust_build_toolchain_config FILENAME)
    file(READ ${CMAKE_CURRENT_FUNCTION_LIST_DIR}/rustlang-env.toml.in RUST_ENV_TEMPLATE)
    file(WRITE ${FILENAME} "# Auto generated by: ${CMAKE_CURRENT_LIST_FILE}\n[env]\n")

    foreach(RUST_CONFIG_ARCH ${ARGN})
        string(CONFIGURE ${RUST_ENV_TEMPLATE} RUST_ENV_CONFIG)
        file(APPEND ${FILENAME} ${RUST_ENV_CONFIG})
    endforeach()

    # For some special cases set an explicit linker too
    if(APPLE)
        set(RUST_CONFIG_LINKER ${CMAKE_C_COMPILER})
    elseif(MSVC)
        set(RUST_CONFIG_LINKER ${CMAKE_LINKER})
    else()
        return()
    endif()
    foreach(RUST_CONFIG_ARCH ${ARGN})
        file(APPEND ${FILENAME} "\n[target.${RUST_CONFIG_ARCH}]\n")
        file(APPEND ${FILENAME} "linker=\"${RUST_CONFIG_LINKER}\"\n")
    endforeach()
endfunction()
if(IOS)
    __rust_build_toolchain_config(${CMAKE_BINARY_DIR}/cargo_home/config.toml aarch64-apple-ios x86_64-apple-ios aarch64-apple-ios-sim)
elseif(CMAKE_SYSTEM_NAME STREQUAL "Darwin")
    __rust_build_toolchain_config(${CMAKE_BINARY_DIR}/cargo_home/config.toml aarch64-apple-darwin x86_64-apple-darwin)
elseif(MSVC)
    __rust_build_toolchain_config(${CMAKE_BINARY_DIR}/cargo_home/config.toml x86_64-pc-windows-msvc)
endif()
...
function(build_rust_archives)
    ...
    list(APPEND RUST_BUILD_CARGO_ENV
        CARGO_HOME=\"${CMAKE_BINARY_DIR}/cargo_home\"
        RUSTC=${RUSTC_BUILD_TOOL}
    )
