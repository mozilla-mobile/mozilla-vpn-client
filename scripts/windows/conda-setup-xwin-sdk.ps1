...
xwin --accept-license --manifest-version "$VS_MANIFEST_VERSION" splat --include-debug-symbols --include-debug-libs --use-winsysroot-style --preserve-ms-arch-notation --disable-symlinks --output "$conda_folder\xwin"

Write-Output("Downloading Microsoft.VisualStudio.Component.VC.Redist.MSM")
python "$PSScriptRoot\fetch-vsix-package.py" --manifest-version "$VS_MANIFEST_VERSION" --output "$conda_folder\xwin" Microsoft.VisualStudio.Component.VC.Redist.MSM
