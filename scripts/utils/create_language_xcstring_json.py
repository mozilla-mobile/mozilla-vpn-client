...
"""
Generate one language's JSON for later combining into AppShortcuts.xcstrings and Localizable.xcstrings

Usage:
    python3 create_language_xcstrings_json.py <strings.yaml> <mozillavpn.xliff> <output_dir>

Example:
    python3 scripts/utils/create_language_xcstring_json.py \
        src/translations/strings.yaml \
        3rdparty/i18n/fr/mozillavpn.xliff \
        tmp/translationSnippets/
"""
...
from xlifftool import guess_locale
...
def build_localizable_snippets(main_strings, locale_translations):
...
def main():
    ...
    # per one language - need xliff file, strings.yaml file, directory to write to
    # combine - need directory with all files, and output file
    ...
    parser.add_argument(
        "xliff_file", help="Path to 3rdparty/i18n/*/mozillavpn.xliff file"
    )
    parser.add_argument(
        "output_dir", help="Directory to write the temporary snippets files into"
    )
    ...
    locale = guess_locale(args.xliff_file)
    locale_translations = load_xliff_translations(args.xliff_file, locale == "en")

    os.makedirs(args.output_dir, exist_ok=True)

    # Want to run these checks, but only need to run once - it fails for all languages, or no languages.
    # Thus, we run it only on one language. And we arbitrarily pick English.
    if locale == "en":
    ...
    localizable = build_localizable_snippets(main_strings, locale_translations)
