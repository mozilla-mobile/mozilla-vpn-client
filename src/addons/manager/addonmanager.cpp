...
void AddonManager::unload(const QString& addonId) {
  ...
  AddonData addon = m_addons[addonId];

  if (addon.m_addon) {
    if (addon.m_addon->enabled()) {
      addon.m_addon->disable();
    }
    ...
    if (m_addonDirectory.getDirectory(&dir)) {
      ...
      QResource::unregisterResource(
          reinterpret_cast<const uchar*>(addon.m_content.constData()),
          mountPath(addonId));
    }
    addon.m_addon->deleteLater();
  }
...
bool AddonManager::validateAndLoad(const QString& addonId,
  ...
  QByteArray* content = &m_addons[addonId].m_content;

  if (!m_addonDirectory.readFile(addonFileName, content)) {
    return false;
  }
  // Hash validation
  if (checkSha256) {
    if (QCryptographicHash::hash(*content, QCryptographicHash::Sha256) !=
        sha256) {
  ...
  if (!loadManifest(QString(":%1/manifest.json").arg(addonMountPath))) {
    QResource::unregisterResource(
        reinterpret_cast<const uchar*>(content->constData()), addonMountPath);
    return false;
  }
