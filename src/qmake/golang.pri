...
golibs.output = ${QMAKE_FILE_BASE}.a
golibs.commands = @echo Building ${QMAKE_FILE_IN} \
    && cd ${QMAKE_FILE_PATH} \
    && GOCACHE=$${GOCACHE} go build -buildmode=c-archive $${GOFLAGS} -v -o $${_PRO_FILE_PWD_}/${QMAKE_FILE_OUT} ${QMAKE_FILE_BASE}.go 
...
gohdr.output = ${QMAKE_FILE_BASE}.h
gohdr.commands = $$escape_expand(\\n)  # force creation of rule
gohdr.clean = ${QMAKE_FILE_BASE}.h
gohdr.depends += ${QMAKE_FILE_BASE}.a
