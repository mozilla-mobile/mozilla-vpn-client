...
static QList<InspectorCommand> s_commands{
    ...
    InspectorCommand{"view_tree", "Sends a view tree", 0,
                     [](DevCmdHandler*, const QList<QByteArray>&) {
                       return DevCmdHandler::getViewTree();
                     }},
...
void DevCmdHandler::recv(const QByteArray& command) {
...
void DevCmdHandler::logEntryAdded(const QByteArray& log) {
...
void DevCmdHandler::networkRequestFinished(QNetworkReply* reply) {
  if (!s_forwardNetwork) {
    return;
  }
...
// static
QString DevCmdHandler::getObjectClass(const QObject* target) {
  if (target == nullptr) {
    return "Unknown";
  }
...
// static
QJsonObject DevCmdHandler::getViewTree() {
  QJsonObject out;
  out["type"] = "qml_tree";
...
// static
QJsonObject DevCmdHandler::serialize(QQuickItem* item) {
  QJsonObject out;
  if (item == nullptr) {
    return out;
  }
...
void DevCmdHandler::itemsPicked(const QList<QQuickItem*>& objects) {
...
// static
void DevCmdHandler::registerCommand(
