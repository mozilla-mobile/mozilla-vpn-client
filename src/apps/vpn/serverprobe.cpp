...
ServerProbe::ServerProbe() { MZ_COUNT_CTOR(ServerProbe); }

ServerProbe::~ServerProbe() { MZ_COUNT_DTOR(ServerProbe); }

void ServerProbe::initialize() {
  ...
  connect(vpn->controller(), &Controller::stateChanged, this,
          &ServerProbe::stateChanged);

  connect(&m_pingTimeout, &QTimer::timeout, this,
          &ServerProbe::maybeSendPings);

  m_refreshTimer.setSingleShot(true);
  connect(&m_refreshTimer, &QTimer::timeout, this, &ServerProbe::start);
  ...
  connect(feature, &Feature::supportedChanged, this, &ServerProbe::start);
...
void ServerProbe::maybeSendPings() {
...
void ServerProbe::recvPing(quint16 sequence) {
...
void ServerProbe::criticalPingError() {
  logger.info() << "Encountered Unrecoverable ping error";
}

qint64 ServerProbe::avgLatency() const {
...
void ServerProbe::setLatency(const QString& pubkey, qint64 msec) {
  m_sumLatencyMsec -= m_latency[pubkey];
  m_sumLatencyMsec += msec;
  m_latency[pubkey] = msec;
}

double ServerProbe::progress() const {
...
void ServerProbe::setCooldown(const QString& publicKey, qint64 timeout) {
  if (timeout <= 0) {
