...
class Tunnel : public QObject {
 ...
 public:
  ...
  /**
   * @brief Rotates the authentication token on a running tunnel
   * @param token The new auth token to hand to the running backend
   * @return true if the token was delivered to the backend, false otherwise
   *
   * Token-authenticated protocols (MASQUE) use this to refresh credentials
   * without tearing down active connections. Protocols that authenticate with
   * a key pair (WireGuard) leave this as a no-op.
   */
  virtual bool rotateToken(const QString& token) {
    Q_UNUSED(token);
    return false;
  }
