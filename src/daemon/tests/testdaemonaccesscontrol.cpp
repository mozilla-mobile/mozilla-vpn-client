...
// A mocked version where we can force the peer id
class MockDaemonAccessControl : public DaemonAccessControl {
 public:
  explicit MockDaemonAccessControl() : DaemonAccessControl() {};

  void setMockPeerId(int peerId) { m_mockPeerId = peerId; }
  int m_mockPeerId = -1;

  int getPeerId(const QLocalSocket* socket) const override { return m_mockPeerId; }
};

void TestDaemonAccessControl::testCommandsWhenInactive() {
  MockDaemonAccessControl dac;
