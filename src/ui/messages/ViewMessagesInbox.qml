...
VPNFlickable {
    ...
    //is accounted for in onFlickContentHeightChanged in VPNFlickable (otherwise it thinks the flickable is at (0,0)
    ...
    onVisibleChanged: {
        if (visible) {
            recalcContentHeightTimer.running = true
        }
        else {
            resetPage()
        }
    ...
    Timer {
        id: recalcContentHeightTimer
        interval: 1
        onTriggered: flickContentHeight = Qt.binding(() => { return layout.implicitHeight })
    }
