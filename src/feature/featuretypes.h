 ...
 * This is a constant value that is determined at compile time and cannot be
 * overridden. This allows for constexpr checks. i.e if constexpr (isEnabled(f))
 * { ... }
 *
 * They should be used if possible. If the flag needs to be determined at
 * runtime, use RuntimeFeature instead.
 */
 ...
 *
 * They should be used if the value cannot be determined at compile time but
 * also doesn't need to be overridden. If the flag needs to be overridden, use
 * OverridableFeature instead.
 */
 ...
 * Overrides are stored in SettingsHolder and take precedence over the default
 * value.
 */
...
/**
 * Toggle an OverridableFeature on or off. This will update the override
 * settings in SettingsHolder. The new value will be returned on the next call
 * to isEnabled for the feature.
 ...
 */
using AnyFeature = std::variant<const ConstantFeature*, const RuntimeFeature*,
