...
void PurchaseIAPHandler::registerProducts(const QByteArray& data) {
...
void PurchaseIAPHandler::unknownProductRegistered(const QString& identifier) {
...
void PurchaseIAPHandler::productsRegistrationCompleted() {
...
void PurchaseIAPHandler::stopProductsRegistration() {
...
PurchaseIAPHandler::Product* PurchaseIAPHandler::findProduct(
...
void PurchaseIAPHandler::addProduct(const QJsonValue& value) {
  if (!value.isObject()) {
...
void PurchaseIAPHandler::stopSubscription() {
  logger.debug() << "Stop subscription";
  m_subscriptionState = eInactive;
  stopProductsRegistration();
}
...
QHash<int, QByteArray> PurchaseIAPHandler::roleNames() const {
...
int PurchaseIAPHandler::rowCount(const QModelIndex&) const {
  if (m_productsRegistrationState != eRegistered) {
    return 0;
  }
...
QVariant PurchaseIAPHandler::data(const QModelIndex& index, int role) const {
...
void PurchaseIAPHandler::computeSavings() {
...
// static
PurchaseIAPHandler::ProductType PurchaseIAPHandler::productTypeToEnum(
    const QString& type) {
...
// static
uint32_t PurchaseIAPHandler::productTypeToMonthCount(ProductType type) {
  switch (type) {
    case ProductYearly:
      return 12;
...
void PurchaseIAPHandler::sortPlans() {
