...
class MasqueTunnelLinux final : public MasqueTunnel {
 ...
 public:
  ...
  // Write a new token to the masque-vpn daemon's stdin. It applies to new
  // streams only. Returns false if the daemon isn't running or the write fails.
  bool rotateToken(const QString& token) override;
