...
bool MasqueTunnelLinux::rotateToken(const QString& token) {
  if (token.isEmpty()) {
    logger.warning() << "Ignoring empty token rotation request";
    return false;
  }
  if (m_daemonProcess.state() != QProcess::Running) {
    logger.warning() << "Cannot rotate token: MASQUE daemon is not running";
    return false;
  }

  // The daemon reads one token per line from stdin. Append a newline so it
  // treats each rotation as a complete token.
  QByteArray payload = token.toUtf8();
  payload.append('\n');

  qint64 written = m_daemonProcess.write(payload);
  if (written != payload.size()) {
    logger.error() << "Failed to write rotated token to MASQUE daemon stdin";
    return false;
  }
  logger.info() << "Rotated MASQUE token delivered to daemon";
  return true;
}
