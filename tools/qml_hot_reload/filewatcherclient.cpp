...
FileWatcherClient::FileWatcherClient(QCoreApplication* app, QObject* parent)
  ...
  connect(&m_webSocket, &QWebSocket::connected, this,
          &FileWatcherClient::onConnected);
  connect(&m_webSocket, &QWebSocket::disconnected, this,
          &FileWatcherClient::onDisconnected);
  connect(&m_fileWatcher, &QFileSystemWatcher::fileChanged, this,
          &FileWatcherClient::onFileChanged);
  connect(m_app, &QCoreApplication::aboutToQuit, this,
          &FileWatcherClient::aboutToQuit);
...
FileWatcherClient::~FileWatcherClient() { m_webSocket.close(); }

void FileWatcherClient::startWatching() {
  qInfo() << "Performing initial scan of:" << m_watchRootPath;
  QDirIterator it(m_watchRootPath, QStringList() << "*.qml", QDir::Files,
                  QDirIterator::Subdirectories);
  while (it.hasNext()) {
    QString filePath = it.next();
    m_fileWatcher.addPath(filePath);
  }
  qInfo() << "Initial scan complete. Watching" << m_fileWatcher.files().count()
          << "QML files.";
}
...
void FileWatcherClient::aboutToQuit() { m_webSocket.close(); }
