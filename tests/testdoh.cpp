...
void TestDOH::basic_data()
{
    ...
    QJsonObject json;
    QTest::addRow("empty") << QJsonDocument(json).toJson() << true << QStringList() << QStringList();

    json.insert("Status", 42);
    QTest::addRow("error status") << QJsonDocument(json).toJson() << true << QStringList() << QStringList();

    json.insert("Status", 0);
    json.insert("Answer", 42);
    QTest::addRow("invalid answer") << QJsonDocument(json).toJson() << true << QStringList() << QStringList();

    QJsonArray answers;
    json.insert("Answer", answers);
    QTest::addRow("empty answer") << QJsonDocument(json).toJson() << true << QStringList() << QStringList();

    answers.append(42);
    json.insert("Answer", answers);
    QTest::addRow("invalid answer object") << QJsonDocument(json).toJson() << true << QStringList() << QStringList();

    QJsonObject answer;
    answers.replace(0, answer);
    json.insert("Answer", answers);
    QTest::addRow("empty answer object") << QJsonDocument(json).toJson() << true << QStringList() << QStringList();

    answer.insert("type", 42);
    answers.replace(0, answer);
    json.insert("Answer", answers);
    QTest::addRow("invalid answer object type") << QJsonDocument(json).toJson() << true << QStringList() << QStringList();
    ...
    answers.replace(0, answer);
    json.insert("Answer", answers);
    QTest::addRow("ipv4") << QJsonDocument(json).toJson() << true << QStringList{"ipv4"} << QStringList();
...
void TestDOH::basic()
{
    ...
    QEventLoop loop;
    connect(r, &DOHRequest::completed, [&](const QStringList &resultIpv4Addresses, const QStringList &resultIpv6Addresses) {
