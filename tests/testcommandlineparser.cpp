...
void TestCommandLineParser::basic_data()
{
    ...
    QTest::addRow("--version") << QStringList{"something", "--version"} << 0 << false;
    QTest::addRow("invalid long option") << QStringList{"something", "--foo"} << 1 << false;
    QTest::addRow("invalid short option") << QStringList{"something", "-f"} << 1 << false;
    QTest::addRow("ui implicit") << QStringList{"something"} << 0 << true;
    QTest::addRow("ui explicit") << QStringList{"something", "ui"} << 0 << true;
    QTest::addRow("invalid command") << QStringList{"something", "foo"} << 1 << false;
    QTest::addRow("ui plus -h") << QStringList{"something", "-h", "ui"} << 0 << false;
