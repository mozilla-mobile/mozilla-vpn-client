...
describe('Server', function() {
  ...
  // it('checking location screen impression telemetry on server view open',
  // async () => {
  //   //Ensures the server view opens to multihop
  //   await vpn.setSetting('serverData',
  //   '{"enter_city_name":"","enter_country_code":"","exit_city_name":"Melbourne","exit_country_code":"au"}');
  //   await
  //   vpn.waitForQueryAndClick(queries.screenHome.SERVER_LIST_BUTTON.visible());
  //   await vpn.waitForQuery(queries.screenHome.STACKVIEW.ready());

  //   const locationSinglehopScreenEvents = await
  //   vpn.waitForGleanValue("impression", "locationSinglehopScreen", "main");
  //   assert.equal(locationSinglehopScreenEvents.length, 1);
  //   const locationSinglehopScreenEventsExtras =
  //   locationSinglehopScreenEvents[0].extra;
  //   assert.equal(singleHopTelemetryScreenId,
  //   locationSinglehopScreenEventsExtras.screen);
  // });
  ...
  //   await vpn.setSetting('serverData',
  //   '{"enter_city_name":"Atlanta","enter_country_code":"us","exit_city_name":"Melbourne","exit_country_code":"au"}');
  //   await
  //   vpn.waitForQueryAndClick(queries.screenHome.SERVER_LIST_BUTTON.visible());
  //   await vpn.waitForQuery(queries.screenHome.STACKVIEW.ready());
  //   await
  //   vpn.waitForQueryAndClick(queries.screenHome.serverListView.SINGLEHOP_SELECTOR_TAB.visible());
  ...
  //     await
  //     vpn.waitForQueryAndClick(queries.screenHome.serverListView.HELP_BUTTON.visible());
  ...
  //     await
  //     vpn.waitForQuery(queries.screenHome.serverListView.HELP_SHEET.opened());

  //     const locationInfoScreenEvents = await
  //     vpn.waitForGleanValue("impression", "locationInfoScreen", "main");
  //     assert.equal(locationInfoScreenEvents.length, 1);
  //     const locationInfoScreenEventsExtras =
  //     locationInfoScreenEvents[0].extra;
  //     assert.equal(locationSheetTelemetryScreenId,
  //     locationInfoScreenEventsExtras.screen);

  //     await
  //     vpn.waitForQueryAndClick(queries.screenHome.serverListView.HELP_SHEET_LEARN_MORE_BUTTON.visible());
  ...
  //     const locationMultihopScreenEvents = await
  //     vpn.waitForGleanValue("impression", "locationMultihopScreen", "main");
  //     assert.equal(locationMultihopScreenEvents.length, 1);
  //     const locationMultihopScreenEventsExtras =
  //     locationMultihopScreenEvents[0].extra;
  //     assert.equal(multiHopTelemetryScreenId,
  //     locationMultihopScreenEventsExtras.screen);
  //   });

  //   it('checking location screen impression telemetry on segment change',
  //   async () => {
  ...
  //     await vpn.waitForQuery(queries.screenHome.STACKVIEW.ready());
  //     await
  //     vpn.waitForQueryAndClick(queries.screenHome.serverListView.MULTIHOP_SELECTOR_TAB.visible());
  ...
  //     //We only need to test the helpTooltipSelected event as it has a
  //     different extra key value (multihop instead of singlehop)
