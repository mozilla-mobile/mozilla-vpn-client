...
describe('IP info', function() {
  ...
  //     const [ { extra: appStepExtras } ] = await
  //     vpn.gleanTestGetValue("impression", "mainScreen", "main")
  //     assert.strictEqual("main", appStepExtras.screen);

  //     // Open IP info panel
  //     await
  //     vpn.waitForQueryAndClick(queries.screenHome.IP_INFO_TOGGLE.visible());
  //     await vpn.waitForQuery(queries.screenHome.IP_INFO_PANEL.ready());
  //     assert.equal(
  //         await vpn.getQueryProperty(
  //             queries.screenHome.IP_INFO_PANEL, 'isOpen'), 'true');

  //     const openedEventsList = await vpn.waitForGleanValue("interaction",
  //     "openConnectionInfoSelected", "main")
  ...
  //     const [ { extra: connectionInfoOpenedExtras } ] = await
  //     vpn.gleanTestGetValue("impression", "connectionInfoScreen", "main")
  //     assert.strictEqual("connection_info",
  //     connectionInfoOpenedExtras.screen);
  ...
  //     const closedEventsList = await vpn.waitForGleanValue("interaction",
  //     "closeSelected", "main") assert.strictEqual(closedEventsList.length,
  //     1); const closedExtras = closedEventsList[0].extra;
