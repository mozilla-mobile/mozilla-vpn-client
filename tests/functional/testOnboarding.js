...
describe('Onboarding', function() {
  ...
  describe('initialize onboarding telemetry tests', () => {
    ...
    //   await
    //   vpn.waitForQuery(queries.screenOnboarding.ONBOARDING_VIEW.visible());
    //   let onboardingStartedEvents = await vpn.waitForGleanValue("outcome",
    //   "onboardingStarted", "main");
    ...
    //   //Ensure the event is only recorded the first time it is shown (glean
    //   store is reset on re-launch) onboardingStartedEvents = await
    //   vpn.gleanTestGetValue("outcome", "onboardingStarted", "main");
    //   assert.equal(onboardingStartedEvents.length, 0);
    // });

    // it('Onboarding completed event is recorded', async () => {
    //   //Make sure data collection is enabled so our metrics don't get deleted
    //   after we complete onboarding await
    //   vpn.setSetting('onboardingDataCollectionEnabled', true);
    ...
    //   let onboardingCompletedEvents = await vpn.waitForGleanValue("outcome",
    ...
    //   const dataCollectionScreenEvents = await
    //   vpn.waitForGleanValue("impression", "dataCollectionScreen", "main");
    //   assert.equal(dataCollectionScreenEvents.length, 1);
    //   const dataCollectionScreenEventsExtras =
    //   dataCollectionScreenEvents[0].extra;
    //   assert.equal(dataScreenTelemetryId,
    //   dataCollectionScreenEventsExtras.screen);

    //   await advanceToSlide(1)

    //   //Verify that getMorePrivacyScreen event is recorded
    //   const getMorePrivacyScreenEvents = await
    //   vpn.waitForGleanValue("impression", "getMorePrivacyScreen", "main");
    //   assert.equal(getMorePrivacyScreenEvents.length, 1);
    //   const getMorePrivacyScreenEventsExtras =
    //   getMorePrivacyScreenEvents[0].extra;
    //   assert.equal(privacyScreenTelemetryId,
    //   getMorePrivacyScreenEventsExtras.screen);

    //   await advanceToSlide(2)

    //   //Verify that installOn5DevicesScreen event is recorded
    //   const installOn5DevicesScreenEvents = await
    //   vpn.waitForGleanValue("impression", "installOn5DevicesScreen", "main");
    //   assert.equal(installOn5DevicesScreenEvents.length, 1);
    //   const installOn5DevicesScreenEventsExtras =
    //   installOn5DevicesScreenEvents[0].extra;
    //   assert.equal(devicesScreenTelemetryId,
    //   installOn5DevicesScreenEventsExtras.screen);

    //   await advanceToSlide(3)

    //   //Verify that connectOnStartupScreen event is recorded
    //   const connectOnStartupScreenEvents = await
    //   vpn.waitForGleanValue("impression", "connectOnStartupScreen", "main");
    //   assert.equal(connectOnStartupScreenEvents.length, 1);
    //   const connectOnStartupScreenEventsExtras =
    //   connectOnStartupScreenEvents[0].extra;
    //   assert.equal(startScreenTelemetryId,
    //   connectOnStartupScreenEventsExtras.screen);
    // });

    // it('Progress bar button events are recorded', async () => {
    //   //We call these more than once throughout the test, lets keep track of
    //   the aggregate and avoid resetting glean let dataUseSelectedEventCounter
    //   = 0 let morePrivacySelectedEventCounter = 0

    //   await vpn.waitForQuery(queries.screenOnboarding.DATA_SLIDE.visible());

    //   //Ensure telemetry is not generated by clicking on the current step
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.STEP_PROG_BAR_DATA_BUTTON.visible());
    //   let dataUseSelectedEvents = await vpn.gleanTestGetValue("interaction",
    //   "dataUseSelected", "main"); assert.equal(dataUseSelectedEvents.length,
    //   0);
    ...
    //   //Ensure telemetry is not generated by clicking on the current step
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.STEP_PROG_BAR_PRIVACY_BUTTON.visible());
    //   let morePrivacySelectedEvents = await
    //   vpn.gleanTestGetValue("interaction", "morePrivacySelected", "main");
    ...
    //   dataUseSelectedEvents = await vpn.gleanTestGetValue("interaction",
    //   "dataUseSelected", "main"); assert.equal(dataUseSelectedEvents.length,
    //   dataUseSelectedEventCounter); let dataUseSelectedEventsExtras =
    //   dataUseSelectedEvents[dataUseSelectedEventCounter - 1].extra;
    //   assert.equal(privacyScreenTelemetryId,
    //   dataUseSelectedEventsExtras.screen);
    ...
    //   //Ensure telemetry is not generated by clicking on the current step
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.STEP_PROG_BAR_DEVICES_BUTTON.visible());
    //   let addDevicesSelectedEvents = await
    //   vpn.gleanTestGetValue("interaction", "addDevicesSelected", "main");
    ...
    //   morePrivacySelectedEvents = await vpn.gleanTestGetValue("interaction",
    //   "morePrivacySelected", "main");
    //   assert.equal(morePrivacySelectedEvents.length,
    //   morePrivacySelectedEventCounter); let morePrivacySelectedEventsExtras =
    //   morePrivacySelectedEvents[morePrivacySelectedEventCounter - 1].extra;
    //   assert.equal(devicesScreenTelemetryId,
    //   morePrivacySelectedEventsExtras.screen);
    ...
    //   dataUseSelectedEvents = await vpn.gleanTestGetValue("interaction",
    //   "dataUseSelected", "main"); assert.equal(dataUseSelectedEvents.length,
    //   dataUseSelectedEventCounter); dataUseSelectedEventsExtras =
    //   dataUseSelectedEvents[dataUseSelectedEventCounter - 1].extra;
    //   assert.equal(devicesScreenTelemetryId,
    //   dataUseSelectedEventsExtras.screen);
    ...
    //   //Start slide progress bar button does not have telemetry (because it
    //   never does anything)
    ...
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.STEP_PROG_BAR_DEVICES_BUTTON.visible());
    //   addDevicesSelectedEvents = await vpn.waitForGleanValue("interaction",
    //   "addDevicesSelected", "main");
    //   assert.equal(addDevicesSelectedEvents.length, 1);
    //   let addDevicesSelectedEventsExtras = addDevicesSelectedEvents[0].extra;
    //   assert.equal(startScreenTelemetryId,
    //   addDevicesSelectedEventsExtras.screen);
    ...
    //   "morePrivacySelected", "main");
    //   assert.equal(morePrivacySelectedEvents.length,
    //   morePrivacySelectedEventCounter); morePrivacySelectedEventsExtras =
    //   morePrivacySelectedEvents[morePrivacySelectedEventCounter - 1].extra;
    //   assert.equal(startScreenTelemetryId,
    //   morePrivacySelectedEventsExtras.screen);
    ...
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.DATA_PRIVACY_LINK.visible());
    //   const privacyNoticeSelectedEvents = await
    //   vpn.waitForGleanValue("interaction", "privacyNoticeSelected", "main");
    //   assert.equal(privacyNoticeSelectedEvents.length, 1);
    //   const privacyNoticeSelectedEventsExtras =
    //   privacyNoticeSelectedEvents[0].extra;
    //   assert.equal(dataScreenTelemetryId,
    //   privacyNoticeSelectedEventsExtras.screen);
    ...
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.DATA_NEXT_BUTTON.visible());
    //   const continueSelectedEvents = await
    //   vpn.waitForGleanValue("interaction", "continueSelected", "main");
    //   assert.equal(continueSelectedEvents.length, 1);
    //   const continueSelectedEventsExtras = continueSelectedEvents[0].extra;
    //   assert.equal(dataScreenTelemetryId,
    //   continueSelectedEventsExtras.screen);
    // });
    ...
    //   await
    //   vpn.waitForQuery(queries.screenOnboarding.PRIVACY_SLIDE.visible());

    //   //Verify that blockAdsEnabled event is recorded
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.PRIVACY_BLOCK_ADS_TOGGLE.visible());
    //   const blockAdsEnabledEvents = await
    //   vpn.waitForGleanValue("interaction", "blockAdsEnabled", "main");
    //   assert.equal(blockAdsEnabledEvents.length, 1);
    //   const blockAdsEnabledEventsExtras = blockAdsEnabledEvents[0].extra;
    //   assert.equal(privacyScreenTelemetryId,
    //   blockAdsEnabledEventsExtras.screen);

    //   //Verify that blockAdsDisabled event is recorded
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.PRIVACY_BLOCK_ADS_TOGGLE.visible());
    //   const blockAdsDisabledEvents = await
    //   vpn.gleanTestGetValue("interaction", "blockAdsDisabled", "main");
    //   assert.equal(blockAdsDisabledEvents.length, 1);
    //   const blockAdsDisabledEventsExtras = blockAdsDisabledEvents[0].extra;
    //   assert.equal(privacyScreenTelemetryId,
    //   blockAdsDisabledEventsExtras.screen);

    //   //Verify that blockTrackersEnabled event is recorded
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.PRIVACY_BLOCK_TRACKERS_TOGGLE.visible());
    //   const blockTrackersEnabledEvents = await
    //   vpn.waitForGleanValue("interaction", "blockTrackersEnabled", "main");
    //   assert.equal(blockTrackersEnabledEvents.length, 1);
    //   const blockTrackersEnabledEventsExtras =
    //   blockTrackersEnabledEvents[0].extra;
    //   assert.equal(privacyScreenTelemetryId,
    //   blockTrackersEnabledEventsExtras.screen);

    //   //Verify that blockTrackersDisabled event is recorded
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.PRIVACY_BLOCK_TRACKERS_TOGGLE.visible());
    //   const blockTrackersDisabledEvents = await
    //   vpn.waitForGleanValue("interaction", "blockTrackersDisabled", "main");
    //   assert.equal(blockTrackersDisabledEvents.length, 1);
    //   const blockTrackersDisabledEventsExtras =
    //   blockTrackersDisabledEvents[0].extra;
    //   assert.equal(privacyScreenTelemetryId,
    //   blockTrackersDisabledEventsExtras.screen);

    //   //Verify that blockMalwareEnabled event is recorded
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.PRIVACY_BLOCK_MALWARE_TOGGLE.visible());
    //   const blockMalwareEnabledEvents = await
    //   vpn.waitForGleanValue("interaction", "blockMalwareEnabled", "main");
    //   assert.equal(blockMalwareEnabledEvents.length, 1);
    //   const blockMalwareEnabledEventsExtras =
    //   blockMalwareEnabledEvents[0].extra;
    //   assert.equal(privacyScreenTelemetryId,
    //   blockMalwareEnabledEventsExtras.screen);

    //   //Verify that blockMalwareDisabled event is recorded
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.PRIVACY_BLOCK_MALWARE_TOGGLE.visible());
    //   const blockMalwareDisabledEvents = await
    //   vpn.waitForGleanValue("interaction", "blockMalwareDisabled", "main");
    //   assert.equal(blockMalwareDisabledEvents.length, 1);
    //   const blockMalwareDisabledEventsExtras =
    //   blockMalwareDisabledEvents[0].extra;
    //   assert.equal(privacyScreenTelemetryId,
    //   blockMalwareDisabledEventsExtras.screen);

    //   //Verify that goBackSelected event is recorded
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.PRIVACY_BACK_BUTTON.visible());
    //   const goBackSelectedEvents = await vpn.waitForGleanValue("interaction",
    //   "goBackSelected", "main"); assert.equal(goBackSelectedEvents.length,
    //   1); const goBackSelectedEventsExtras = goBackSelectedEvents[0].extra;
    //   assert.equal(privacyScreenTelemetryId,
    //   goBackSelectedEventsExtras.screen);
    ...
    //   //Verify that continueSelected event is recorded
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.PRIVACY_NEXT_BUTTON.visible());
    //   const continueSelectedEvents = await
    //   vpn.waitForGleanValue("interaction", "continueSelected", "main");
    //   assert.equal(continueSelectedEvents.length, 1);
    //   const continueSelectedEventsExtras = continueSelectedEvents[0].extra;
    ...
    //   await
    //   vpn.waitForQuery(queries.screenOnboarding.DEVICES_SLIDE.visible());

    //   if (await
    //   vpn.getQueryProperty(queries.screenOnboarding.DEVICES_DEVICE_TYPE_TOGGLE.visible(),
    //   'selectedIndex') == 0) {
    //     //[Android, Windows, Linux] Starting with Android - switch to iOS and
    //     back to Android
    //     //Verify that showIosQrSelected event is recorded
    //     await
    //     vpn.waitForQueryAndClick(queries.screenOnboarding.DEVICES_TOGGLE_BTN_IOS.visible());
    //     const showIosQrSelectedEvents = await
    //     vpn.waitForGleanValue("interaction", "showIosQrSelected", "main");
    //     assert.equal(showIosQrSelectedEvents.length, 1);
    //     const showIosQrSelectedEventsExtras =
    //     showIosQrSelectedEvents[0].extra;
    //     assert.equal(devicesScreenTelemetryId,
    //     showIosQrSelectedEventsExtras.screen);

    //     //Verify that showAndroidQrSelected event is recorded
    //     await
    //     vpn.waitForQueryAndClick(queries.screenOnboarding.DEVICES_TOGGLE_BTN_ANDROID.visible());
    //     const showAndroidQrSelectedEvents = await
    //     vpn.waitForGleanValue("interaction", "showAndroidQrSelected",
    //     "main"); assert.equal(showAndroidQrSelectedEvents.length, 1); const
    //     showAndroidQrSelectedEventsExtras =
    //     showAndroidQrSelectedEvents[0].extra;
    //     assert.equal(devicesScreenTelemetryId,
    //     showAndroidQrSelectedEventsExtras.screen);
    ...
    //   //Verify that goBackSelected event is recorded
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.DEVICES_BACK_BUTTON.visible());
    //   const goBackSelectedEvents = await vpn.waitForGleanValue("interaction",
    //   "goBackSelected", "main"); assert.equal(goBackSelectedEvents.length,
    //   1); const goBackSelectedEventsExtras = goBackSelectedEvents[0].extra;
    //   assert.equal(devicesScreenTelemetryId,
    ...
    //   //Verify that continueSelected event is recorded
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.DEVICES_NEXT_BUTTON.visible());
    //   const continueSelectedEvents = await
    //   vpn.waitForGleanValue("interaction", "continueSelected", "main");
    //   assert.equal(continueSelectedEvents.length, 1);
    //   const continueSelectedEventsExtras = continueSelectedEvents[0].extra;
    ...
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.START_START_AT_BOOT_TOGGLE.visible());
    ...
    //   const connectOnStartupEnabledEventsExtras =
    //   connectOnStartupEnabledEvents[0].extra;
    //   assert.equal(startScreenTelemetryId,
    //   connectOnStartupEnabledEventsExtras.screen);
    ...
    //   const connectOnStartupDisabledEventsExtras =
    //   connectOnStartupDisabledEvents[0].extra;
    //   assert.equal(startScreenTelemetryId,
    //   connectOnStartupDisabledEventsExtras.screen);

    //   //Verify that goBackSelected event is recorded
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.START_BACK_BUTTON.visible());
    //   const goBackSelectedEvents = await vpn.waitForGleanValue("interaction",
    //   "goBackSelected", "main"); assert.equal(goBackSelectedEvents.length,
    //   1); const goBackSelectedEventsExtras = goBackSelectedEvents[0].extra;
    //   assert.equal(startScreenTelemetryId,
    ...
    //   //For the next test we need data collection enabled due to a race
    //   condition between recording
    //   //the getStartedSelected event and data collection being disabled at
    //   onboarding completion await
    //   vpn.setSetting('onboardingDataCollectionEnabled', true)

    //   //Verify that getStartedSelected event is recorded
    //   await
    //   vpn.waitForQueryAndClick(queries.screenOnboarding.START_NEXT_BUTTON.visible());
    //   const getStartedSelectedEvents = await
    //   vpn.waitForGleanValue("interaction", "getStartedSelected", "main");
    //   assert.equal(getStartedSelectedEvents.length, 1);
    //   const getStartedSelectedEventsExtras =
    //   getStartedSelectedEvents[0].extra; assert.equal(startScreenTelemetryId,
    //   getStartedSelectedEventsExtras.screen);
    // });
