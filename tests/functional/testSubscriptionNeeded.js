...
describe("subscription needed tests", function() {
  ...
  describe('subscription needed related telemetry tests', function() {
    ...
    describe('subscription screen', function() {
      ...
      //   const  subNeededImpression = await
      //   vpn.waitForGleanValue("impression", "subscriptionNeededScreen",
      //   "main"); assert.strictEqual(subNeededImpression.length, 1) const
      //   enterEmailViewEventExtras = subNeededImpression[0].extra;
      ...
      //   await
      //   vpn.waitForQueryAndClick(queries.screenSubscriptionNeeded.SUBSCRIPTION_NEEDED_GET_HELP.visible());
      ...
      //   await
      //   vpn.waitForQueryAndClick(queries.screenSubscriptionNeeded.SUBSCRIPTION_NEEDED_TERMS_OF_SERVICE.visible());
      //   await vpn.testLastInteractionEvent({
      ...
      //   await
      //   vpn.waitForQueryAndClick(queries.screenSubscriptionNeeded.SUBSCRIPTION_NEEDED_PRIVACY_NOTICE.visible());
      //   await vpn.testLastInteractionEvent({
      ...
      //   await
      //   vpn.waitForQueryAndClick(queries.screenSubscriptionNeeded.SUBSCRIPTION_NEEDED_BUTTON.visible());
      //   const subscriptionNeededViewEvents = await
      //   vpn.waitForGleanValue("impression", "subscriptionNeededScreen",
      //   "main"); const subscriptionNeededViewEventExtras =
      //   subscriptionNeededViewEvents[0].extra; assert.strictEqual(screen,
      //   subscriptionNeededViewEventExtras.screen);
      // });

      // it("restore purchase event is recorded", async () => {
      //   // Make the "Already a subscriber?" button visible
      //   await
      //   vpn.setQueryProperty(queries.screenSubscriptionNeeded.SUBSCRIPTION_NEEDED_RESTORE_PURCHASE,
      //   "visible", true)
      //   // Click the "Already a subscriber?" button
      //   await
      //   vpn.waitForQueryAndClick(queries.screenSubscriptionNeeded.SUBSCRIPTION_NEEDED_RESTORE_PURCHASE.visible());
      //   await vpn.testLastInteractionEvent({
      ...
      //   await
      //   vpn.waitForQueryAndClick(queries.screenSubscriptionNeeded.SUBSCRIPTION_NEEDED_BUTTON.visible());
      //   const startedEvents = await vpn.waitForGleanValue("outcome",
      //   "subscriptionStarted", "main");
      ...
      //   const completedEvents = await vpn.waitForGleanValue("outcome",
      //   "subscriptionCompleted", "main");
    ...
    describe('loading screens', function() {
      ...
      //   await
      //   vpn.waitForQuery(queries.screenInBrowserSubscriptionLoading.SUBSCRIPTION_LOADING_VIEW.visible());

      //   // Wait for the impression metric.
      //   const impressionEvent = await vpn.waitForGleanValue("impression",
      //   "continueInBrowserScreen", "main");
      ...
      //   await
      //   vpn.waitForQuery(queries.screenInBrowserSubscriptionLoading.SUBSCRIPTION_LOADING_VIEW.visible());
      //   // Wait for the impression metric - which indicates the view is done
      //   loading. const impressionEvent = await
      //   vpn.waitForGleanValue("impression", "continueInBrowserScreen",
      //   "main"); assert.strictEqual(impressionEvent.length, 1);
      //   assert.strictEqual(impressionEvent[0].extra.screen, screen);

      //   // Click on the "Cancel" button
      //   await
      //   vpn.waitForQueryAndClick(queries.screenInBrowserSubscriptionLoading.SUBSCRIPTION_LOADING_CANCEL.visible());

      //   // Check subscription cancelled event is recorded.
      //   const eventList = await vpn.waitForGleanValue("outcome",
      //   "subscriptionFailed", "main"); assert.strictEqual(eventList.length,
      //   1); assert.strictEqual(eventList[0].extra.reason,
