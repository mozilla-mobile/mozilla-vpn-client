...
#define VERIFY_IS_INVALID(x) QVERIFY(!(x).isValid())
#define VERIFY(x) QVERIFY(x)
#define VERIFY_IS_FALSE(x) QVERIFY(!(x))
...
struct testTwiceBiggerType {
  static void run() {
    ...
    QVERIFY(sizeof(typename detail::TwiceBiggerType<T>::Type) == 2 * sizeof(T));
    QVERIFY(bool(std::is_signed_v<typename detail::TwiceBiggerType<T>::Type>) ==
...
void test() {
  ...
  QVERIFY(detail::IsSupported<T>::value);
  const bool isTSigned = std::is_signed_v<T>;
  QVERIFY(bool(isTSigned) == !bool(T(-1) > T(0)));

  testTwiceBiggerType<T>::run();

  using unsignedT = std::make_unsigned_t<T>;

  QVERIFY(sizeof(unsignedT) == sizeof(T));
  QVERIFY(std::is_signed_v<unsignedT> == false);
  ...
  for (size_t i = 0; i < sizeof(T) * CHAR_BIT - 1; i++) {
    QVERIFY((unsignedMinValue & bit) == 0);
    bit <<= 1;
  }
  QVERIFY((unsignedMinValue & bit) == (isTSigned ? bit : unsignedT(0)));
  QVERIFY(unsignedMaxValue == unsignedT(~unsignedMinValue));
...
void TestCheckedInt::tests() {
  ...
  test<qlonglong>();
  test<qptrdiff>();
  // qreal is not supported! test<qreal>();
  test<qsizetype>();
  test<quint8>();
  test<quint16>();
  test<quint32>();
...
static TestCheckedInt s_testCheckedInt;
