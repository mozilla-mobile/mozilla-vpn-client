...
## App Intents and Widgets for iOS

App Intents - the Apple framework which powers features like suggested actions and "Hey Siri, turn on Mozilla VPN" - require using Apple's native translation system. Widgets also requires these files. iOS handles much of the work of App Intents and Widgets without opening our app, and thus translations must be in .xcstrings format. The `generate_xcstrings.py` Python script converts translations from our translation system into the xcstrings files that iOS requires. This conversion is done every time the iOS app is built, to provide the freshest strings possible.

There are two types of string categories (from the `strings.yaml` file) that the `generate_xcstrings.py` script works with:
1. `iosAppIntentsActivate`, `iOSAppIntentsDeactivate`, etc.: (written to `AppShortcuts.xcstrings`): These strings are the range of utterances which can turn the VPN on and off via Siri. They cover many ways that a user may ask Siri to toggle the VPN. For this category, translations are not necessarily identical. Ideally, translations cover the range of reasonable utterances a user may say to try to turn the VPN on or off. Thus, different languages may require a different number of strings in this category. Additionally, each string requires a placeholder for `{applicationName}`, as iOS requires the application's name to be part of the command. There are checks in the `generate_xcstrings.py` to ensure the placeholders exist.
1. `iosAppIntentsMain` (written to `Localizable.xcstrings`): This category is for all App Intents strings that are not part of the prior category. Each string in this category should be explicitly written as a `LocalizedStringResource` in a swift file, as well as the reverse - every explicitly translated string in a `LocalizedStringResource` should be in this category. There are checks in the `generate_xcstrings.py` to ensure this remains true.

> Important: The `generate_xcstrings.py` script is run when the Xcode project file is created via Cmake, **not** when the build is actually compiled. If translations are added or updated, the iOS build folder must be removed and re-created to include the new translations. This is unfortunately necessary so they are available at the point in the build when iOS wants to extract them. More details in a comment in `src/translations/CMakeLists.txt`.
